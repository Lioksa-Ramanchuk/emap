...
int main()
{
    ...
    do
    {
        ...
        cout << "  1 ———————> калькулятор для работы с целыми числами\n";
        ...
        do
        {
            ...
            switch (answer)
            {
            case '1':; // функция: калькулятор для работы с целыми числами - НИКИТА КАРЕБО
                break;
            case '2': stringCalculator(); //                               - ВЛАД ГОРОЩЕНЯ
                break;
            case '3':; // функция: часы: вывод текущего времени            - ОЛЕГ ТОЧИЛО
                break;
