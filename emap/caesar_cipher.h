...
// Вывод текста после(де-)шифрования
void putText(std::string& strNewText, const eCipherMode& CIPHER_MODE = eCipherMode::ENCRYPT_MODE);

// Запись текста в файл после (де-)шифрования
void putTextInFile(std::string& strNewText, const eCipherMode& CIPHER_MODE = eCipherMode::ENCRYPT_MODE);
